---
name: openclaw-vps-doctor
description: "Диагностика и обслуживание бота «Иваныч» (openclaw на VPS в Москве): бот молчит, не отвечает, потерял инструменты, сломался после обновления, не работает поиск/голосовые/календарь/бэкапы. Используй при любой жалобе на работу бота, а также перед правкой openclaw.json, обновлением версии и настройкой прав агента."
---

# Иваныч: диагностика и обслуживание

Операционный справочник по личному боту на VPS. Собран из разборов реальных
поломок 05–07.2026. **Главное правило: проверяй фактами, не выводами** — ни моими
прошлыми, ни отчётами самого бота (он описывает намерения как результат).

## Доступ и устройство

```bash
ssh clawd-vps-tail          # Tailscale, пользователь clawd; связь рвётся — повторяй с -o ConnectTimeout=45
```

**Запасной вход, когда SSH недоступен:** консоль VNC в панели TimeWeb. Пускает
только `root` с паролем (у `clawd` пароля нет — доступ всегда был по ключу).
Пароль root задаётся там же в панели. Держи это в голове заранее: если сеть
на машине сломана, консоль — единственный путь, а разбираться с паролем в
момент аварии поздно. Порт 22 наружу закрыт намеренно (`ListenAddress` только
на tailscale-адресе и loopback) — публичный IP для входа не годится.

| Что | Где |
|---|---|
| Служба | `systemctl --user openclaw-gateway` (user unit, не системный) |
| Конфиг | `~/.openclaw/openclaw.json` |
| CLI | `~/.npm-global/bin/openclaw` (в PATH может не быть) |
| Правила бота | `~/.openclaw/workspace/` — **вложенный git-репо**, ветка `bot-rules` |
| База знаний | `~/emmbase/`, бот пишет **только** в `inbox/` |
| Скрипты и cron | `~/.openclaw/scripts/`, копии в репо `deploy/scripts/` |
| Ключи моделей | SQLite `~/.openclaw/agents/main/agent/openclaw-agent.sqlite`, таблица `auth_profile_store` |

Логи: `journalctl --user -u openclaw-gateway`. Шум `exec: elevated` отфильтровывай.

---

## 1. «Бот молчит» — порядок проверки

Четыре известных режима отказа. Идти сверху вниз, **не перезапуская вслепую**.

```bash
ssh clawd-vps-tail 'systemctl --user is-active openclaw-gateway
  ls ~/.openclaw/telegram/ingress-spool-default/ | wc -l
  tail -5 ~/.openclaw/logs/telegram-watchdog.log
  timeout 120 ~/.npm-global/bin/openclaw channels status'
```

| Признак | Причина | Лечение |
|---|---|---|
| файлы в ingress-spool старше 15 мин, `update-offset` не двигается | умер потребитель очереди внутри процесса | рестарт gateway (перезапуск канала не помогает) |
| серия `fetch timeout … getMe`, нет успешного трафика | протух пул соединений после обрыва Tor | рестарт gateway |
| `channels status` → `stopped`, `error: restart-loop breaker tripped` | 4+ неудачных старта за 5 мин подавили автозапуск канала | подождать 5 мин без падений → один чистый рестарт |
| служба `failed`, в логе `Invalid config … status=78/CONFIG` | битый конфиг | починить конфиг, затем `systemctl --user reset-failed` перед стартом |

Сторож `telegram-watchdog.sh` (cron `*/5`) ловит все четыре: неактивную службу,
зависшую очередь, серию таймаутов, подавленный канал, плюс холостые рестарты
health-monitor. Уведомляет владельца сам, с первой неудачи восстановления.

⚠️ После рестарта убедись, что очередь разошлась (`ingress-spool` пуст) и в логе
есть `outbound send ok`. «Служба active» ничего не доказывает.

---

## 2. «У бота нет инструментов»

Жалобы вида «недоступны `memory_search`, `curl`, `web_search`». Смотри в логе
строки `[agents/tool-policy]` — там прямо написано, что и каким слоем вырезано.

**Три независимых слоя:**

1. `tools.sandbox.tools.deny` — по умолчанию режет `cron, gateway, nodes`.
2. `tools.sandbox.tools.alsoAllow` — **белый список**: всё, чего в нём нет,
   вырезается в песочнице.
3. `gateway sender owner-only` — `cron` доступен **только владельцу**
   (`commands.ownerAllowFrom`).

⚠️ **Из-за третьего слоя консольный тест врёт.** `openclaw agent --agent main`
не имеет личности владельца → `cron` «недоступен», даже когда всё настроено
верно. Такие вещи проверяются **только реальным сообщением в Telegram**.

**Песочница работает без сети** (`NetworkMode: none`) — это защита, а не поломка.
`curl`/`wget` внутри неё бесполезны, ставить их в образ бессмысленно.
Интернет только через host-инструменты: `web_fetch` (замена curl), `web_search`.

Почему путаница: в переписке с владельцем `curl` работает — там `tools.elevated`
выполняет команды на хосте. В задачах по расписанию этих прав нет.

**В cron-задачах отправлять ничего не надо** — настроено `announce -> telegram`,
итоговый ответ агента уходит сам. Инструмент `message` в песочнице закрыт.

---

## 3. Сеть: что доступно из Москвы

VPS у TimeWeb (РФ). Перед внесением хоста в `NO_PROXY` **проверяй обе ветки**:

```bash
curl -s --max-time 20 -o /dev/null -w "прямо: %{http_code}\n" https://ХОСТ/
curl -s --max-time 40 -x http://127.0.0.1:8118 -o /dev/null -w "через Tor: %{http_code}\n" https://ХОСТ/
```

| Сервис | Напрямую | Через Tor | Как ходит |
|---|---|---|---|
| api.deepgram.com, api.minimax.io | ✅ | ✅ | напрямую, в `NO_PROXY` |
| api.search.brave.com | ✅ | ✅ | напрямую |
| openrouter.ai | ✅ 200 | ✅ | **напрямую, гео-блока больше нет** (сверено 07.09.2026) |
| api.openai.com | ❌ 403 | ✅ 401 | через privoxy `127.0.0.1:8118` |
| api.tavily.com | ❌ 403 | ✅ | **плагин игнорирует прокси** → выключен |
| api.telegram.org | ❌ | — | transparent SOCKS: iptables + redsocks |

**Codex (`chatgpt.com/backend-api`) — работает, но держится на двух костылях.**
openclaw ходит туда мимо всех настроек прокси, поэтому трафик заворачивается
прозрачно: `codex-tor-route.sh` (cron `17 * * * *`) держит `iptables REDIRECT`
адресов chatgpt.com на redsocks и **закрывает IPv6** (`ip6tables REJECT` на
`2a06:98c1::/32`) — иначе соединение уходит по IPv6 мимо правил v4.

Настройки прокси на этот путь **не влияют**, не трать на них время:
`HTTPS_PROXY`/`ALL_PROXY` в systemd, те же переменные перед CLI,
`models.providers.openai.request.proxy`, конфиг плагина `codex`.

⚠️ Сообщение **«You've reached your Codex subscription usage limit» врёт.** Так
openclaw переводит и 403 по региону, и 429 `insufficient_quota` от мёртвого
платёжного ключа. Прежде чем верить — проверь личный кабинет ChatGPT (лимит
обычно не тронут) и сделай прямой запрос к API (см. п.8).

### WARP перехватывает весь DNS — причина `EAI_AGAIN` (найдено 06.09.2026)

На машине стоит Cloudflare WARP (`warp-svc`). Он перехватывает **все** UDP-запросы
к порту 53 — включая явно адресованные `8.8.8.8` — и гонит их через свой DoH-прокси.
Прокси периодически виснет, и тогда не резолвится **ничего**.

Так выглядят приступы в логе бота: `getaddrinfo EAI_AGAIN api.minimax.io`
(149 раз за 14 дней, вспышками по 30–60 в сутки), следом `All models failed`.

**Как отличить за 10 секунд** — сравни UDP и TCP:

```bash
timeout 6 nslookup api.minimax.io 8.8.8.8   # UDP: FAIL во время приступа
dig +tcp +short @8.8.8.8 api.minimax.io     # TCP: работает всегда
```

UDP не работает ни к одному серверу, а TCP к тому же серверу отвечает — значит
перехват, а не сеть и не провайдер. Подтверждение — растущие счётчики:
`journalctl -u warp-svc | grep doh_err_timeout` (за 40 минут наблюдения +792).

⚠️ **Добавлять резервные резолверы бесполезно** — WARP перехватывает и их.
Проверено: с `77.88.8.8` и `8.8.8.8` в списке приступы продолжались.

**Решено 07.09.2026: WARP остановлен и снят с автозапуска** (`systemctl disable --now
warp-svc`). Результат сразу после: DNS 25 из 25 проверок без сбоя, ноль `EAI_AGAIN`
в логе, ответ агента 9–10 секунд вместо 30–150. Функционально ничего не потеряно —
MiniMax, gonka и Telegram ходят напрямую (проверено кодами 404/401/302).

⚠️ Останавливать WARP **только через консоль VNC** — на живой системе по SSH
это обрубает управляющий канал (см. п.8). Если он снова понадобится, поднимать
его тоже стоит с доступом к консоли под рукой.

---

## 4. Модели и ключи

**Все ключи владельца подписочные.** Проверять их обычным API-вызовом
бессмысленно: «insufficient balance», «quota exceeded», «usage limit» там штатный
ответ, а не признак мёртвого ключа.

- ChatGPT Plus **не покрывает** платформенный ключ `sk-proj-…` — это отдельный
  продукт с отдельной оплатой. На сервере лежат оба доступа, они не связаны.
- MiniMax Coding Plan (`sk-cp-`) работает через `api.minimax.io/anthropic`.
- «Empty response» от MiniMax лечится сбросом раздутой сессии (`/new`), не тарифом.

Рабочая цепочка (сверено запросами 07.09.2026): primary `minimax/MiniMax-M2.7` →
`gonka/MiniMaxAI/MiniMax-M2.7` → `gonka/moonshotai/Kimi-K2.6` →
`openrouter/google/gemini-2.5-flash`.

Состояние провайдеров на 07.09.2026, каждый проверен прямым запросом:

| Провайдер | Состояние | Проверка |
|---|---|---|
| gonka (`router.mingles.ai`) | ✅ жив, бесплатный, отвечает | вернул валидный ответ на `chat/completions` |
| minimax | ✅ жив, подписка, но страдает от приступов DNS | 131k выдачи, `timeoutSeconds` срезан 150 → **30** |
| openrouter | ✅ жив с 07.09.2026 | был `403 Key limit exceeded` при `limit: 10`, `usage: 10.007` и **`limit_reset: null`** — потолок на самом ключе без периода сброса, деньги на аккаунте ни при чём. Владелец включил ежемесячный сброс, ключ ожил. При повторе смотри `limit_reset`, а не баланс |
| openai `gpt-5.5` | ❌ `You have no credits remaining` | платёжный `sk-proj-` не оплачен, из цепочки убран |
| zai (`api.z.ai`) | ❌ `Insufficient balance` | ключ в конфиге живой, денег нет. `doctor --fix` включает его сам — не путай с рабочим |

⚠️ **`timeoutSeconds` у провайдера — компромисс, а не «чем меньше, тем лучше».**
При сбое DNS бот ждал 150 секунд перед переходом к следующей модели — отсюда
«отвечает с задержкой». Срезал до 30 — переписка стала быстрой, но **сломались
тяжёлые cron-задачи**: «Утренний-прогноз» упал шесть раз подряд с
`fetch timeout after 30000ms`. В логе видно почему: обычные вызовы задачи идут
за 1.7–2.3 с, а финальная генерация не укладывается в 30 с.

Рабочее значение — **90 секунд** (с 09.09.2026). Проверяй обе стороны:
задержку ответа в чате и статус задач планировщика после изменения.

⚠️ **Не верь диагнозу бота даже когда он звучит технично.** На это падение он
ответил владельцу: «isolated-сессия не видит сеть, скрипт ходит за данными
через интернет — отсюда тайм-аут» и предложил `cron update --session current`.
Лог показывает `fetch timeout ... url=https://api.minimax.io/anthropic/v1/messages`
— таймаут на вызове модели, сеть ни при чём. Совет бота не только не помог бы,
но и заставил задачу писать в чат. Команды `cron update` вообще **не существует** —
есть `cron edit` (patch полей), `cron get --json`, `cron run <id>` (ставит в очередь,
результат смотри через `cron get`, а не в выводе).

### ⛔ Отчёт о сбое задачи уходит туда же, куда её результат

13.09.2026 в публичный канал «Формула роста» улетело «Automation failed 8 times,
Cause: timeout» — не прогноз, а служебное уведомление. Openclaw шлёт отчёт о
сбое по адресу `delivery.to` задачи; настройки `cron.*` это не меняют. Для
любой задачи с доставкой в канал/группу **сразу** задавай отдельный адрес:

```bash
openclaw cron edit <id> --failure-alert-to 215087477   # или --no-failure-alert
```

Проверка, что именно ушло, без чтения Telegram: `cron get <id> --json` →
`lastDeliveryStatus: not-delivered` при `lastFailureNotificationDeliveryStatus:
delivered` = в чат ушёл отчёт о сбое, а не результат.

### MiniMax душит задачи планировщика, а не переписку

За неделю 09–13.09: **семь 429 и семнадцать таймаутов по 90 с** — все строго в
минуты старта задач (02:01, 06:01, 17:01 UTC), в диалоге ни одного. Тяжёлый
системный промпт задачи (после подъёма `bootstrapMaxChars` — ~38 000 символов)
плюс большое задание не укладываются в лимиты подписки. Лечение — не крутить
таймауты, а дать задачам другую модель и лёгкий контекст:

```bash
openclaw cron edit <id> --model openrouter/google/gemini-2.5-flash \
  --fallbacks "gonka/MiniMaxAI/MiniMax-M2.7,minimax/MiniMax-M2.7" --light-context
```

**`modelPolicy.allow` при этом должен пускать провайдера** — иначе
`Model override "…" is not allowed by modelPolicy.allow`. Рабочая политика:
`["minimax/*","gonka/*","openrouter/*"]` (маски допускаются, пустой список =
разрешено всё). Список, который сгенерировал `doctor --fix`, этому мешал.

Недельному прогнозу нужны данные на 7 дней, а `astro-daily.sh` даёт один:
добавлен `astro-weekly.sh` (cron `40 6 * * 0` UTC) → `/emmbase/life/прогнозы/
_астро-данные-неделя.md`, задание задачи ссылается на него и запрещает веб-поиск
(старый текст велел «расчёт онлайн, попробуй wild.ru» — вопреки правилам).
Первый успешный прогон за 8 недель — 13.09.2026, 319 с, доставлен владельцу.

### OpenRouter: Google и OpenAI закрыты по ToS, деньги ни при чём

Два разных отказа с одинаковым итогом. **402** — кончились предоплаченные
средства на *аккаунте* (`/api/v1/credits`: `total_usage ≥ total_credits`), при
этом лимит *ключа* мог быть в порядке (`limit_remaining` > 0). **403 «violation of
provider Terms Of Service»** — после пополнения: Google (`google/*`) и OpenAI
(`openai/*`) через OpenRouter отказывают с нашего адреса, и напрямую, и через Tor
(13.09.2026, три попытки). Проходят `deepseek/deepseek-chat-v3-0324` (текст) и
`qwen/qwen-2.5-vl-72b-instruct` (зрение — проверено картинкой). На них переведены:
фолбэк чата, heartbeat, задачи планировщика, распознавание картинок; генерация
картинок — `flux-schnell` первым. Прежде чем ставить модель OpenRouter в конфиг,
прогони её реальным запросом с VPS — таблица «работает/не работает» живёт не
дольше недели.

### Два механизма памяти = вызовы уходят не туда

`plugins.slots.memory = openclaw-mem0`, но `memory-core` тоже был включён и
регистрировал `memory_search`/`memory_get`. В ядре конфликт имён решается так:
**второй регистрант отбрасывается** (`plugin tool name conflict (<id>)` — назван
тот, кого отбросили). Отбрасывался mem0 → явные вызовы памяти шли во встроенное
хранилище, а не в qdrant с реальными записями. Лечение — выключить дубль
(`plugins.entries.memory-core.enabled=false`, убрать из `plugins.allow`);
задача «Memory Dreaming Promotion» принадлежит memory-core и исчезает вместе с
ним, у mem0 свой `skills.dream`. Проверка: после старта в логе ноль
`tool name conflict`.

### Мелочи, которые молчали

- **`permission-watchdog.sh` никогда не уведомлял**: делал `source .env`, а `.env`
  не shell-файл (строка «*** Tavily MCP (free…)» ломает разбор). Переведён на
  файл токена, как остальные сторожа; проверяет теперь и каталоги (`secrets/`
  должен быть 700, был 775). `PW_DRY_RUN=1` — прогон без отправки.
- **Deepgram уходил на локальный whisper молча** (`[deepgram-empty-or-fail]` без
  причины). Теперь `[deepgram-fail] curl=<rc> http=<код> body=<240 символов>`.
- **«memory pressure … threshold=750 MiB» по 288 раз в сутки** — не утечка (RSS
  стабильно ~840 МиБ) и не настройка openclaw: порог = `MemoryHigh` юнита × 0.5
  (`DEFAULT_HEAP_WARNING_RATIO`), а в юните стояло 1500M с прежнего, меньшего
  сервера. Поднято до `MemoryHigh=2500M`, `MemoryMax=3G` (хост 8 ГБ) → порог 1250.
  ⚠️ Файл юнита с **CRLF** — `sed 's/^MemoryMax=2G$/…/'` не совпадёт, бери `.*`.

⚠️ **Codex ищет профиль строго с id `openai:manual`** (иначе `Codex app-server
auth profile "openai:manual" was not found`). По умолчанию там лежал мёртвый
платёжный ключ — из-за этого подписка Plus не использовалась вообще. Сейчас под
этим id лежит **копия OAuth-профиля подписки** (`type: oauth`, план `plus`).
Не «наводи порядок», заменяя его на api_key — сломаешь Codex.

Ключи живут в SQLite, **не** в `auth-profiles.json` (там остались старые копии —
полезны как источник при миграции). Восстановление без интерактива:

```bash
printf '%s\n' "$KEY" | openclaw models auth paste-api-key --provider <имя>
```

OAuth-профили (Codex) через `paste-token` не восстанавливаются, а `login` требует
TTY. Переносить напрямую: `auth_profile_store`, `store_key='primary'`,
`store_json.profiles` — структура совпадает со старым `auth-profiles.json`.

---

## 5. Правка конфига и обновление

- **Перед правкой** — копия: `cp openclaw.json openclaw.json.bak-$(date +%s)`.
- **Плагины не принимают произвольные поля.** Ключ Brave кладётся в
  `plugins.entries.brave.config.webSearch.apiKey` + нужна секция
  `tools.web.search = {enabled, provider, maxResults, timeoutSeconds}`.
  Неверное поле роняет старт с `status=78/CONFIG`.
- **Overlay провайдера** возможен только для встроенных; для плагинных
  (`openai-codex`) требует `baseUrl` и `models`, иначе старт падает.
- **Не более ~4 рестартов за 5 минут** — иначе `restart-loop breaker` подавит
  автозапуск канала Telegram (см. п.1). Между правками делай паузы.
- После падений: `systemctl --user reset-failed openclaw-gateway` перед стартом.
- Ждать порт, а не «спать наугад»:
  `for i in $(seq 1 20); do ss -ltn | grep -q 18789 && break; sleep 10; done`

**Обновление openclaw** (грабли 2026.7.x):
1. `npm` целится в `/usr` → нужен `--prefix /home/clawd/.npm-global`.
2. Старт **ждёт подтверждения с клавиатуры** на предупреждениях о неустановленных
   плагинах — под systemd выглядит как вечное зависание без логов. Убрать из
   `plugins.allow`/`entries` то, что не установлено.
3. Ключи провайдеров **не мигрируют** (см. п.4).
4. Каждая неудачная попытка берёт аренду миграции на ~5 минут — рестарты подряд
   бесполезны.

**Обновление 2026.7.1-2 → 2026.9.2 (пройдено 06.09.2026, порядок важен).**

Каталог модулей защищён (`root:clawd`), а в sudoers запрещён `npm` — ставить
надо из root-шелла, модули остаются root-owned, защита не снимается:

```bash
sudo bash -c 'npm install -g --prefix /home/clawd/.npm-global openclaw@ВЕРСИЯ \
  && chown -R root:clawd /home/clawd/.npm-global/lib/node_modules'
```

1. **Сначала сними сторожа с cron** — `telegram-watchdog.sh` поднимет шлюз посреди
   установки, и он повиснет на несовместимых плагинах. Крон вернуть сразу после.
2. Конфиг 2026.7 для 2026.9 **невалиден** (`status=78/CONFIG`), нужен
   `openclaw doctor --fix`. Он мигрирует чисто, ничего не теряя:
   `env/*` → `env/vars/*` (все ключи и прокси на месте),
   `agents.defaults.models` → `modelPolicy.allow`,
   `imageGenerationModel` → `mediaModels.image`,
   `tools.media.audio.models` → `tools.media.models`,
   `gateway.nodes.denyCommands` → `gateway.nodes.commands.deny`.
   Реально снимаются с поддержки: `compaction.maxHistoryShare`,
   `controlUi.allowInsecureAuth`, `tailscale.resetOnExit`.
   Профиль Codex переименовывается `openai-codex:auto.*` → `openai:auto.*`,
   но `openai:manual` (см. п.4) сохраняется — Codex продолжает его находить.
3. Плагины — отдельной командой `openclaw plugins update --all`. `brave` при этом
   встаёт обратно на старую версию, ему нужен явный спек:
   `openclaw plugins install "@openclaw/brave-plugin@ВЕРСИЯ"`.
4. **`mem0` в 2026.9 требует согласия на доступ к переписке** и без него молча не
   грузится (`plugins list` покажет предупреждение). Включается через
   `openclaw plugins enable openclaw-mem0 --accept-capabilities` — решение
   владельца, не агента.
5. Порядок: снять сторожа → установить ядро → `doctor --fix` → `config validate`
   → плагины → старт с ожиданием порта → вернуть сторожа.

Проверка после: `config validate`, `channels status`, живой вызов агента, пустой
`ingress-spool`. Старт занимает ~20–25 секунд; «служба active» ничего не доказывает.

⚠️ **После обновления проверь маркер молчания.** В 2026.9.2
`SILENT_REPLY_TOKEN = "NO_REPLY"`. Если правила бота велят отвечать другим словом,
служебные ответы heartbeat перестают глушиться и **сыплются владельцу в чат**
(«Disk 58%, inbox empty, no alerts. SILENT» каждые полчаса). Найдено 07.09.2026:
`AGENTS.md` требовал `SILENT`, `BOOT.md` — `NO_REPLY`, правила противоречили друг другу.

```bash
grep -rn "NO_REPLY\|SILENT" ~/.openclaw/workspace/*.md      # в правилах
grep -ohE 'SILENT_REPLY_TOKEN ?= ?"[^"]*"' ~/.npm-global/lib/node_modules/openclaw/dist/tokens-*   # в ядре
```

Токен должен быть **всем** содержимым ответа. Приписанный в конец фразы
(`Disk 58%. NO_REPLY`) не распознаётся — сообщение уходит целиком.

⚠️ **После обновления проверь распознавание картинок.** `doctor --fix` сворачивает
`tools.media.audio.models` в `tools.media.models` с тегами возможностей — и записи
для `image` там не оказывается. Распознавание уходит к провайдеру по умолчанию
(OpenAI) с мёртвым ключом, в логе `[media-understanding] image: failed reason=ProviderAuthError`,
а бот отвечает владельцу отговоркой вроде «не поддерживается формат» — потому что
содержимого он не получил. Симптом обманчив: похоже на проблему с файлом, а дело в авторизации.

Лечится явной записью рядом с аудио (аудио не трогать, оно работает):

```json
{"provider": "openrouter", "model": "google/gemini-2.5-flash", "capabilities": ["image"]}
```

Модели в `models.providers` объявлены как `input: ["text"]` — minimax, gonka и zai
картинок не видят в принципе, зрячая модель нужна отдельная.

⛔ **Не выноси разделы из `AGENTS.md` в отдельный `TOOLS.md`.** Соблазн велик: файл
разросся до 41 КБ, в ядре есть `DEFAULT_TOOLS_FILENAME`, а `CLAW_BOOTSTRAP_FILE_NAMES`
перечисляет `TOOLS.md`. **Но в контекст сессии он не попадает.** Рантайм собирает
правила из другого списка:

```bash
grep -rhoE "WORKSPACE_BOOTSTRAP_FILENAMES ?= ?\[[^]]{0,220}\]" ~/.npm-global/lib/node_modules/openclaw/dist
# → [AGENTS.md, SOUL.md, IDENTITY.md, USER.md, BOOTSTRAP, MEMORY] — TOOLS.md отсутствует
```

`workspace.bootstrapFiles` в схеме — про заготовки из пакетов (`packageRelativePath`),
а не про подключение своего файла. Проверено 07.09.2026: разделение молча вынесло
24 КБ инфраструктурных правил из головы бота и было откачено.

⛔ **Память бота перебивает правила — ищи причину там, а не только в `.md`.**
Полгода бот слал владельцу служебные строки с хвостом `SILENT` вместо молчания.
Правку правил это не вылечило: в `mem0` лежала запись `category=rule` —
«запрещено отправлять `NO_REPLY`», остаток со старой версии, где маркер назывался
иначе. Бот честно исполнял память, а не документацию.

```bash
# искать по всей памяти, а не только в правилах
curl -s -X POST http://localhost:6333/collections/mem0_768d/points/scroll \
  -H "Content-Type: application/json" -d '{"limit":200,"with_payload":true}' \
  | grep -o '"data": "[^"]*"' | grep -i "<искомое>"
```

Удалять точечно через `points/delete` по id, предварительно сохранив дамп с
вектором (`points` с `with_vector:true`) — так запись восстановима.

⚠️ **Не спеши списывать странный ответ на «консоль не грузит правила».** 07.09 я
так и решил, когда `openclaw agent --agent main` не назвал маркер молчания —
а он цитировал ту самую ошибочную память. Правила консоль видит.

### Служебные сообщения heartbeat: чините конфигом, а не правилами

Ни правка `AGENTS.md`, ни удаление ошибочной памяти спам не остановили — модель
продолжала считать «Disk 57%, uptime 21.8h, no anomalies» достойным отправки.
В ядре для этого есть два рычага, и они надёжнее любых формулировок:

```jsonc
// channels.defaults — гасит отчёты «всё в порядке», тревоги оставляет
"heartbeatVisibility": { "showOk": false, "showAlerts": true }

// agents.defaults.heartbeat — вне этих часов опрос не запускается вовсе
"activeHours": { "start": "08:00", "end": "22:00", "timezone": "Europe/Moscow" }
```

До 08.09.2026 в `heartbeat` была задана только `model`, поэтому бот будил
владельца отчётами в 4 утра. Полная схема: `every`, `activeHours`, `model`,
`session`, `target`, `directPolicy`.

Правило в `AGENTS.md` всё равно держи верным (маркер `NO_REPLY` и запрет на
статус-строки), но не рассчитывай, что модель его исполнит без опоры в конфиге.

⚠️ `heartbeatVisibility.showOk` **не гасит текст модели** — он про собственный
индикатор openclaw. Проверено: с `showOk:false` heartbeat всё равно отправил
статус. Работает только `directPolicy`.

### ⛔ Правила молча обрезаются на 20 000 символов

Самая дорогая находка 08.09.2026. В логе каждого прогона:

```
workspace bootstrap file AGENTS.md is 28286 chars (limit 20000); truncating in injected context
```

`AGENTS.md` разросся до 28 286 символов — **8 286 символов хвоста до бота не
доходили вовсе**. Там жили раздел «Inbox — КРИТИЧНО» и правила самоправки.
Любая правка в конце файла бессмысленна, пока предел не поднят: чинишь текст,
которого модель не видит.

```bash
# проверка — должно быть пусто
journalctl --user -u openclaw-gateway --since "-30 min" | grep truncating
```

Лечится в `agents.defaults`: `bootstrapMaxChars` (на файл, по умолчанию 20000)
и `bootstrapTotalMaxChars` (на все файлы). Поставлено 34000 и 52000 при сумме
файлов правил ~38 700 символов. Цена — примерно 3–4 тысячи токенов в каждом
запросе; альтернатива, если жалко, — сокращать `AGENTS.md`, но тогда решай,
какими правилами жертвуешь, а не позволяй ядру резать по счётчику символов.

---

## 6. Голос и транскрипция

Звонки — AMR-WB **моно**, обрабатывает host-скрипт `transcribe-audio-watcher.sh`
(cron ежеминутно). Голосовые из Telegram идут другим путём — встроенным
media-understanding.

- DeepGram **обязателен** `language=ru`: без него `nova-3` даёт пустой транскрипт.
- Диаризация: `diarize=true` на моно видит одного спикера. Нужно
  `diarize_model=latest&utterances=true`, совмещать с `diarize` нельзя (400).
- Собирать из `results.utterances[]`, а не из плоского `transcript`, и **склеивать**
  подряд идущие реплики одного говорящего (иначе 491 обрывок на 24 минуты).
- Голосовой якорь `~/.openclaw/media/voice-anchor-ruslan.ogg` клеится встык
  (без паузы — пауза ломает связку) и отрезается **пословно**.

---

## 7. Бэкапы

Два репозитория, оба уходят в приватный `autorusland-ai/openclaw-backup`:

| Ветка | Что | Особенность |
|---|---|---|
| `main` | `~/.openclaw` + `openclaw.json` | шифруется git-crypt (магия `\x00GITCRYPT\x00`) |
| `bot-rules` | `~/.openclaw/workspace` | вложенный репо, нужен `git -C` |

⚠️ В `workspace/` лежат `media/` (записи звонков клиентов) и `openclaw-config.json`
с ключами открытым текстом — исключены через `.gitignore`. Перед пушем прогоняй
поиск по префиксам `sk-proj-`, `sk-or-v1`, `gsk_`, `AIzaSy`, `GOCSPX`, `github_pat_`.

⚠️ **`openclaw.json` не попадает в автоматические бэкапы.** В
`openclaw-autocommit.sh` он занесён в `BLOCK_RE` с обоснованием «не покрыт
фильтрами git-crypt» — **обоснование устарело**: в `.gitattributes` есть строка
`openclaw.json filter=git-crypt`, и в истории файл лежит зашифрованным (блоб
начинается с `\0GITCRYPT\0`, проверено 07.09.2026). Пока фильтр не поправлен,
после каждой правки конфига коммить его руками:

```bash
git -C ~/.openclaw add openclaw.json && git -C ~/.openclaw commit -m "..."
```

Хук `gitleaks` на коммите отработает и подтвердит, что открытых ключей нет.

`git-crypt status` без аргумента виснет на untracked-папках — проверяй точечно.
Локальные снимки: `pre-update-backup.sh` (cron вс, ротация 8).

---

## 8. Как работать, чтобы не сломать

### ⛔ Три команды, которые обрубают доступ к серверу (проверено 06–07.09.2026)

Сеть на этой машине держится на рантайм-настройках, которых **нет ни в одном
конфиге**: WARP и Tailscale выставляют себе маршруты, метки и `rp_filter` при
старте. Любая команда, сбрасывающая это на живой системе, убивает управляющий
канал — SSH, Tailscale, всё сразу. Лечится только консолью VNC в панели TimeWeb.

| Команда | Что происходит | Как чинить |
|---|---|---|
| `systemctl restart warp-svc` | остаются висеть правила policy-routing и `CONNMARK`, ведущие в мёртвый тоннель. Симптом: TCP-рукопожатие проходит, но данные не возвращаются; Tailscale `tx N rx 0` | перезагрузка сервера из панели |
| `sysctl --system` | перечитывает **все** файлы разом и сбивает рантайм-параметры сети, из-за чего WARP снова ломает маршрутизацию | из консоли **`systemctl stop warp-svc`**, затем `systemctl restart tailscaled`. Проверено: `restart tailscaled` и `rp_filter=2` по отдельности **не помогают** — лечит только остановка WARP |
| перезагрузка (любая) | **гонка при старте**: sshd поднимается раньше Tailscale, привязка `ListenAddress 100.76.223.48` падает, демон живёт на одном loopback. Симптом: мгновенный `Connection refused` отовсюду при живом `tailscale ping` | из консоли `systemctl restart ssh`; постоянно — `net.ipv4.ip_nonlocal_bind=1` в `/etc/sysctl.d/` |

Правило: одиночное значение ставь точечно (`sysctl -w ключ=значение`), никогда не
перезагружай весь набор. Сетевые службы на живой системе не перезапускай вообще —
планируй перезагрузку, когда есть доступ к консоли.

⚠️ **`Connection refused` и `Connection timed out` — разные диагнозы.** Refused
значит сеть до машины дошла и ядро ответило RST: сервер жив, не слушает порт.
Timed out — сломан обратный маршрут. Не путай, они ведут к разным причинам.

- **Проверяй эмпирически.** Гипотезу про причину подтверждай измерением
  (curl, прямой вызов API, лог), а не рассуждением. Половина разборов в этом
  проекте опровергла «очевидный» диагноз.
- **Смотри, уходит ли трафик вообще.** Если сомневаешься, доходит ли запрос до
  сервиса — посмотри SYN-пакеты во время попытки:
  `sudo timeout 45 tcpdump -nn -i any 'tcp[tcpflags] & tcp-syn != 0 and port 443'`.
  Пусто = openclaw отказал локально (кулдаун, отсутствующий профиль), и чинить
  надо не сеть. Этот приём сэкономил бы час возни с прокси.
- **Сравнивай прямой запрос и через Tor** — так отличаешь гео-блок (403 прямо,
  осмысленный ответ через прокси) от настоящей проблемы с ключом или квотой.
  Осмысленная ошибка API (400 «Stream must be set to true») — признак, что
  авторизация и регион в порядке.
- **Не верь отчётам бота** — он описывает намерения как факты.
- **Изолированный тест вместо боевого**: копия скрипта с подменёнными путями в
  `/tmp` и `TG_TOKEN=TEST_INVALID_NO_SEND`, чтобы не слать владельцу мусор.
- **Осторожно с `pkill -f`** из ssh-команды: шаблон совпадёт с собственной
  строкой запуска и убьёт сессию. Спасает класс символов: `git-cryp[t] statu`.
- **Правки на VPS дублируй в репо** `deploy/scripts/` и сверяй хешем.
- **Скрипты защищены от записи** (с 15.08.2026, после того как бот сломал рабочий
  скрипт и пытался обновить платформу). `~/.openclaw/scripts/` — `root:clawd`,
  файлы 750; `~/.npm-global/lib/node_modules` — `root:clawd`; в sudoers запрещены
  `npm/npx/apt/apt-get/dpkg/pip/pip3/snap`. Выкладка теперь в два шага:
  `scp … :/tmp/x.sh` → `sudo cp /tmp/x.sh ~/.openclaw/scripts/ && sudo chown root:clawd … && sudo chmod 750 …`.
  Логи скриптов лежат в том же каталоге и остались writable — дозапись в
  существующий файл прав на каталог не требует, cron из-за защиты не ломается.
- Помни про часовые пояса: cron на VPS в UTC, владелец живёт по Москве.

## История разборов

- [WORK-SUMMARY-2026-07-26.md](../../../WORK-SUMMARY-2026-07-26.md) — восстановление после
  двухсуточного молчания, аудит, обновление платформы.
- [WORK-SUMMARY-2026-07-27.md](../../../WORK-SUMMARY-2026-07-27.md) — права инструментов,
  поиск, Codex, повторяющиеся задачи.
- [deploy/WORK-HISTORY.md](../../../deploy/WORK-HISTORY.md) — хронология с мая.
