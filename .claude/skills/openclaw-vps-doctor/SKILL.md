...
Сторож `telegram-watchdog.sh` (cron `*/5`) ловит первые два и неактивную службу,
шлёт уведомления владельцу. Третий режим он **не ловит** — проверяй руками.
...
**Codex (`chatgpt.com/backend-api`) — тупик.** Прямой запрос даёт 403 по региону,
через Tor работает, но openclaw ходит мимо прокси и показывает 403 как
**«You've reached your Codex subscription usage limit»**. Формулировке не верить:
проверь личный кабинет — лимит обычно не тронут. Перелогин не помогает.
Не помогают: `HTTPS_PROXY`/`ALL_PROXY` в systemd, те же переменные перед CLI,
`models.providers.openai.request.proxy`; у плагина `codex` настройки прокси нет.
