...
if ! systemctl --user is-active --quiet openclaw-gateway; then
  ...
  # [4] Канал ПОДАВЛЕН защитой от циклических падений. Ловушка в том, что снаружи
  # всё выглядит здоровым: служба active, очередь пуста, сеть исправна — просто
  # канал выключен намеренно после серии неудачных стартов (инцидент 27.07,
  # причина — правки конфига подряд). Само не рассасывается.
  if [ -z "$reason" ]; then
    suppressed=$(journalctl --user -u openclaw-gateway --since "-15 minutes" --no-pager 2>/dev/null \
      | grep -c "autostart suppressed" || true)
    if [ "${suppressed:-0}" -gt 0 ]; then
      reason="канал Telegram подавлен защитой от циклических падений (autostart suppressed)"
    fi
  fi
