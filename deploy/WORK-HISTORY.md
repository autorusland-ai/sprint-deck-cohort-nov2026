...
# Цикл 27.07.2026 — права инструментов, поиск, Codex

Полный разбор: **[../WORK-SUMMARY-2026-07-27.md](../WORK-SUMMARY-2026-07-27.md)**.

| Область | Изменение |
|---|---|
| Права агента | белый список песочницы расширен (поиск, память, `web_fetch`, `cron`); `gateway`/`nodes` остаются закрыты |
| Поиск | Brave включён с новым ключом, Tavily выключен (гео-блок + игнорирует прокси) |
| Модели | cron-задачи переведены на MiniMax, `gpt-5.5` в конец цепочки |
| Правила бота | разделы про `web_fetch` вместо curl и про самостоятельную настройку расписания |

**Ключевые факты, которые дорого выяснять заново:**

1. Инструменты режут **три** слоя политики; `cron` доступен только владельцу,
   поэтому из CLI проверить его нельзя — только сообщением в Telegram.
2. Песочница работает **без сети** (`NetworkMode: none`) — `curl` там бесполезен.
3. Codex недоступен с этого VPS: 403 по региону, который openclaw показывает как
   «исчерпан лимит подписки». Перелогин не помогает.
4. Более 4 рестартов за 5 минут подавляют автозапуск канала Telegram.
